...
//	intervalNumPrecision = 600;
    ...
    ODE<Real> dynamics({"x1", "15*sin(x0)", "1", "0"}, vars);
...
//	setting.prepare();
	...
	int steps = 200;
	Interval init_x0(0.3, 0.3), init_x1(-0.9, -0.9), init_t(0), init_u(0);
...
//		cout << "Neural network taylor remainder: " << rm1 << endl;
		...
		if (result.status == COMPLETED_SAFE || result.status == COMPLETED_UNKNOWN)
		{
			...
			if (print || safe_sets) {
				for (int i = 0; i < 4; ++i) {
					cout << setprecision(10) <<  final_box[i].inf() << " ";
					cout << setprecision(10) <<  final_box[i].sup() << " ";
				}
				cout << "\n";
			}

			// safe loss for each timestep
			double safe_loss = final_box[1].sup() * final_box[1].sup() - 100;
...
//			cout << "Flowpipe taylor remainder: " << initial_set.tmv.tms[0].remainder << "     " << initial_set.tmv.tms[1].remainder << endl;
	...
	if (print) {
		...
		plot_setting.setOutputDims("t", "u0");
		plot_setting.plot_2D_interval_GNUPLOT("./outputs/", benchmark_name + "_u0", result.tmv_flowpipes, setting);
	}
