...
# import core as trajectory_sampler
import onnx
import onnx2torch
import torch
...
class seq_linear:
    ...
    def get_action(self, o, t):
        ...
        return np.array(
            [
                np.inner(self.linear_policy[t], modified_o),
            ]
        )
...
class neural_model_without_read:
    ...
    def get_action(self, o, t):
        ...
        obs = torch.from_numpy(o)
        return self.model(
            torch.tensor(obs).type(torch.float64).unsqueeze(0), evaluate=True
        )
    ...
    def sample_from_initial_states(self, initial_states, env_str, horizon, plot=False):
        ...
        for idx, initial_state in enumerate(initial_states):
            ...
            while t < horizon and done is not True:
                action = self.get_action(obs, t)[0]
                obs, r, done, _ = gym_env.step(action)
                if (t + 1) % sample_interval == 0:
                    path.append(obs)
                complete_path.append(obs)
