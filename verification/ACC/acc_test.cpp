...
//	intervalNumPrecision = 600;
    ...
    ODE<Real> dynamics({"x1","u0","0"}, vars);
...
//	setting.prepare();
	...
	int steps = 1;
	Interval init_x0(-1.01766, -0.678024), init_x1(0.247261, 0.482329), init_u(0);
	// Interval init_x0(-1.1, -0.9), init_x1(-0.1,0.1), init_u(0);
	...
	// no unsafe set
	vector<Constraint> safeSet = {Constraint("x0 + 0.8", vars), 
								 Constraint("-0.8 - x0", vars), 
								 Constraint("x1 - 0.35", vars),
								 Constraint("0.35 - x1", vars)};
...
//		cout << "Neural network taylor remainder: " << rm1 << endl;
		...
		if (result.status == COMPLETED_SAFE || result.status == COMPLETED_UNKNOWN)
		{
			...
			for (int a = 0; ; a++) {
				double x = -1.01262 + 0.03 * a;
				for (int b = 0; ; b++){
					double y = 0.254216 + 0.03 * b;
						vector<Constraint> safeSet = {Constraint("x0 + " +  to_string(-1.0 * x), vars), 
								 Constraint(to_string(x) + " - x0", vars), 
								 Constraint("x1 - " + to_string(y), vars),
								 Constraint(to_string(y) + " - x1", vars)};
						cout << "x: " << x << " y: " << y << endl;
						cout << "test result " << result.fp_end_of_time.safetyChecking(safeSet, setting.tm_setting, setting.g_setting) << endl;
					if (y > 0.491766) {
						break;
					}
				}
...
//			cout << "Flowpipe taylor remainder: " << initial_set.tmv.tms[0].remainder << "     " << initial_set.tmv.tms[1].remainder << endl;
	...
	if (print) {
		...
		plot_setting.plot_2D_interval_GNUPLOT("./outputs/", benchmark_name + "_x0", result.tmv_flowpipes, setting);
