...
int main(int argc, char *argv[])
{
	...
	string benchmark_name = "safe_pendulum";
	bool print = false;
...
//	intervalNumPrecision = 600;
	...
	int x2_id = vars.declareVar("t");
	int u_id = vars.declareVar("u");

	int domainDim = numVars + 1;

	// Define the continuous dynamics.
    ODE<Real> dynamics({"x1", "15 * sin(x0) + 3 * u", "1", "0"}, vars);
...
//	setting.prepare();
	...
	for (int iter = 0; iter < steps; ++iter) {
		...
		// cout << "before polar setting" << endl;
