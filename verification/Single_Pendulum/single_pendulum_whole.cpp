...
//	setting.prepare();
	...
	int steps = 80;
	Interval init_x0(1.0, 1.2), init_x1(0.0,0.2), init_t(0), init_u(0);
...
//		cout << "Neural network taylor remainder: " << rm1 << endl;
		...
		if (result.status == COMPLETED_SAFE || result.status == COMPLETED_UNKNOWN)
		{
			...
			if (iter >= 9) {
				double safe_loss = (max(relu_my(0.01 - final_box[0].inf()), relu_my(final_box[0].sup() - 1.0)));
				total_safe_loss += safe_loss;
				if (print) {
			...
			if (iter == steps - 1) {
				// reach loss
				vector<double> target_set_lbs = {-0.05, -0.05};
				vector<double> target_set_ubs = {0.05, 0.05};
				...
				double rloss = reach_loss(final_lbs, final_ubs, target_set_lbs, target_set_ubs);
				total_reach_loss += rloss;
				if (print) {
					cout << "reach loss at iter " << iter << " is " << rloss << "\n";
				}
			}
