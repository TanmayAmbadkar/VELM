...
def eval_controller_lagrange(
    ...
    reshaped_controller = np.reshape(controller, (num_of_controller, -1))
    for seq, one_controller in enumerate(reshaped_controller):
        ...
        f.writelines(line)
        f.close()
    cmd = [config["eval_program"], f"controller_{index}"]
    cmd.extend(args)
    ...
    if args == []:
        ...
        linear_model = mse.seq_linear(
            num_of_controller, config["iters_run"], controller_sz, controller
        )
...
def eval_controller_lagrange(cmd_first_part, controller, index, args=None):
    ...
    for seq, controller in enumerate(controllers):
        ...
        f.writelines(lines)
        f.close()
    cmd = ["./obstacle_mid", f"controller_{index}"]
    # print(cmd)
    if args is not None:
        cmd += args
...
def run_lagrange(cmd_first_part, params, env_str, num_traj, neural_agent):
    import pdb
    ...
    nn = mse.neural_model(neural_agent)
