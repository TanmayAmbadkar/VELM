...
class ControllerPriorGMM(object):
    """
    A dynamics prior encoded as a GMM over [x_t, u_t] points.
    See:
        S. Levine*, C. Finn*, T. Darrell, P. Abbeel, "End-to-end
        training of Deep Visuomotor Policies", arXiv:1504.00702,
        Appendix A.3.
    ...
    def update(self, X, U):
        ...
        # Compute cluster dimensionality.
        Do = X.shape[2] + U.shape[2]  # TODO: Use Xtgt.

        # Create dataset.
        N = self.X.shape[0]
        xu = np.reshape(np.c_[self.X[:, :T, :], self.U[:, :T, :]], [T * N, Do])
