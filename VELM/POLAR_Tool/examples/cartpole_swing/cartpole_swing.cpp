...
{
	double x_loss = max(my_relu(-1 * x_inf - 0.9), my_relu(x_sup - 0.9));
	double theta_loss = max(my_relu(-1 * theta_inf - 1.5), my_relu(theta_sup - 1.5));
...
int main(int argc, char *argv[])
{
	...
	ifstream dynamics_file("./results/learned_dynamics/Marvelgymnasium_cartpole_swing-v1/model.txt");
