...
class Dataset(object):
    def __init__(self, data_root, single_sample=None, file_list=None):
        self.spec_list = []
        self.grammar_list = []
        self.sample_specs = []
        self.file_names = []
    ...
    def load_spec_list(self, fname):
        with open(self.data_root + '/' + fname, 'r') as f:
    ...
    def setup(self, classname):
        ...
        if self.single_sample is None:
            ...
            with open(self.data_root + '/' + self.file_list, 'r') as f:
                for row in f:
                    self.file_names.append(row.strip())
                    self.load_spec_list(row.strip())
        else:
            self.file_names.append(self.single_sample)
            self.load_spec_list(self.single_sample)
