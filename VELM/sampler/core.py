...
def do_rollout(
    ...
    while ep < num_traj:
        ...
        # seeding
        if base_seed is not None:
            seed = base_seed + np.random.randint(100000)
            env.set_seed(seed)
            np.random.seed(seed)
        ...
        while t < horizon and (ignore_done or done is not True):
            # import pdb
            ...
            a = np.array(a)
            # a = np.clip(a, -3, 3)
            next_o, r, terminated, truncated, env_info_step = env.step(a)
        ...
        if t >= horizon:
            ...
            # if abs(path["observations"][-1][0]) < 0.1 and abs(path["observations"][-1][1]) < 0.1 and abs(path["observations"][-1][2]) < 0.1 and abs(path["observations"][-1][3]) < 0.1:
            #     # import pdb
            #     # pdb.set_trace()
            paths.append(path)
