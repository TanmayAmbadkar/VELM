...
class LanguageModel(object):
    def __init__(self, vocabulary_size, embedding_size, num_layers, num_hidden, mode='train'):
        ...
        with tf.compat.v1.variable_scope("rnn"):
            def make_cell():
                cell = tf.nn.BasicRNNCell(self.num_hidden)
                cell = tf.nn.DropoutWrapper(cell, output_keep_prob=self.keep_prob)
                return cell

            cell = tf.nn.MultiRNNCell([make_cell() for _ in range(self.num_layers)])
